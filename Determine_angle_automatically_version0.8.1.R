...
man_orient_file <- 'NTM_s30_man_orient.myrmidon'

### automatically oriented myrmidon file name
auto_orient_file <- 'NTM_s30_auto_orient.myrmidon'
...
data_list         <- list (paste(dir_data,man_orient_file,sep=''))
...
###create space
s <- tracking_data$createSpace("leamas")
...
tddURI <- tracking_data$addTrackingDataDirectory(s$ID,paste(dir_data,track_data_name,sep=''))
tracking_data$save(paste(dir_data,auto_orient_file,sep='')) # file now exists
...
tracking_data$save(paste(dir_data,auto_orient_file,sep='')) 
